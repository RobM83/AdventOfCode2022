...
func main() {
	...
	size := sizeOfFolderToRemove(&folderSizes)
	fmt.Println(size)
}
...
func sizeOfFolderToRemove(folderSizes *map[string]int) int {
	needed := 30000000
	fileSystem := 70000000
	free := fileSystem - (*folderSizes)["/"]
	folderToDeleteSize := fileSystem
	for folder, size := range *folderSizes {
		if free+size >= needed {
			if size < folderToDeleteSize {
				folderToDeleteSize = size
			}
			fmt.Printf("Folder to delete: %s, size %d\n", folder, size)
		}
	}
	return folderToDeleteSize
}
