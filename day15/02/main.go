...
func main() {
	...
	frequency := getTuningFrequency(sensors, coords)
...
func getTuningFrequency(sensors map[coord]*sensor, coords map[coord]string) int {
	position := coord{0, 0}
	spot := &sensor{}
	...
	for {
		...
		for c, s := range sensors {
			spot = s
			if s.distance >= diff(c.x, position.x)+diff(c.y, position.y) { //Spot is covered by non beacon distress signal
				nonBeacon = true //Could also be a beacon or signal, but not the distress beacon
				break
			}
		}

		if !nonBeacon {
			fmt.Println("Found distress beacon at", position.x, position.y)
			break //Found distress beacon
		}

		//Move to next position, diff between position and sensor is next step (reach)
		deltaX := diff(spot.coord.x, position.x)
		deltaY := diff(spot.coord.y, position.y)
		step := spot.distance - deltaY + deltaX + 1 //Walk over X-axis first

		//(between 0 and 4000000)
		//if position.x+step > 20 {
		if position.x+step > 4000000 {
			position.x = 0
			position.y++
		} else {
			position.x += step
	...
	frequency := position.x*4000000 + position.y
