...
func main() {
	...
	dropSand(cave, coord{500, 0}, abyss+2)
...
func printCave(cave map[coord]string) {
	...
	fmt.Println("Max-y	", yCoords[len(yCoords)-1]+2)
}
...
func checkNextCoord(cave map[coord]string, start coord, maxDepth int) (coord, bool) {
	...
	if _, ok := cave[coord{499, 1}]; ok {
		if _, ok := cave[coord{500, 1}]; ok {
			if _, ok := cave[coord{501, 1}]; ok {
				abyss = true
			}
		}
	}
